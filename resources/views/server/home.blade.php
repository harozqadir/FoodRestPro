...
@section('content')
    <h2 class="text-center mb-4">{{ __('words.Server Dashboard - Tables') }}</h2>
                ...
                <a href="{{ route('server.foods', ['id' => $table->id]) }}" class="text-decoration-none">
                    <div class="card shadow-sm border-0 rounded-xl"
                        ...
                        <div class="card-body text-center rudaw-font">
                            ...
                            <h5 class="card-title">{{ __('words.Table') }} #{{ $table->table_number }}</h5>
                            @if ($table->status === 'ordered')
                                <span class="badge bg-danger text-light fs-5 px-3 py-2">{{ __('words.Reserved') }}</span>
                            @else

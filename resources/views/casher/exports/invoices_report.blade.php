{{-- filepath: resources/views/casher/exports/invoices_report_pdf.blade.php --}}
...
<head>
    <meta charset="UTF-8">
    <title>{{ __('words.Invoices Report') }}</title>
...
<body>
    <div class="container">
        ...
        <!-- Title -->
        <h2>{{ __('words.Invoices Report') }}</h2>
        <div class="filter-info">
            <strong>{{ __('words.From') }}:</strong>
            {{ $from ? Carbon::parse($from)->format('Y-m-d') : __('words.All') }}
            &nbsp;|&nbsp;
            <strong>{{ __('words.To') }}:</strong>
            {{ $to ? Carbon::parse($to)->format('Y-m-d') : __('words.All') }}
            &nbsp;|&nbsp;
            ...
            <tbody>
                @forelse($invoices as $invoice)
                    <tr>
                        ...
                        <td>
                            <span
                                class="badge badge-light">{{ $invoice->table->table_number ?? __('words.N/A') }}</span>
                        </td>
                        <td>
                            <span
                                class="badge badge-info">{{ $invoice->creator->username ?? __('words.Unknown') }}</span>
                        </td>
                        <td>
                            <span class="badge badge-primary">{{ number_format($invoice->total_price) }}
                                {{ __('words.IQD') }}</span>
                        </td>
                        <td>
                            @if ($invoice->status == 1)
                                <span class="badge badge-warning">{{ __('words.Unpaid') }}</span>
                            @else
                                <span class="badge badge-success">{{ __('words.Paid') }}</span>
                            @endif
                        </td>
                        <td>
                            <span class="badge badge-light">{{ $invoice->created_at->format('Y-m-d H:i') }}</span>
                        </td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="6" class="text-center text-muted py-4">
                            {{ __('words.No invoice records found.') }}
                        </td>
                    </tr>
                @endforelse
            </tbody>
            <tfoot>
                <tr style="background:#f7f7f7; font-weight:bold; font-size: 16px;">
                    <td colspan="6" style="text-align:center;">
                        <span style="margin-right:24px;"><strong>{{ __('words.Total Sales') }}:</strong>
                            {{ __('words.IQD') }} {{ number_format($totalSales) }}</span>
                        <span style="margin-right:24px;"><strong>{{ __('words.Total Invoices') }}:</strong>
                            {{ $invoices->count() }}</span>
                        <span><strong>{{ __('words.Unpaid') }}:</strong> {{ $unpaidCount }}</span>
                    </td>
                </tr>
            </tfoot>
        </table>
